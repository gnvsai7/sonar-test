...
value=$(echo $record | jq -r '.component.measures[0].value')
echo $value
...
if [[ "$value" = "$A" ]]; then
python3 /var/lib/jenkins/workspace/github-webhook/cliq-a.py
elif [[ "$value" = "$B" ]]; then    
python3 /var/lib/jenkins/workspace/github-webhook/cliq-b.py
elif [[ "$value" = "$C" ]]; then
python3 /var/lib/jenkins/workspace/github-webhook/cliq-c.py
elif [[ "$value" = "$D" ]]; then 
python3 /var/lib/jenkins/workspace/github-webhook/cliq-d.py
else
python3 /var/lib/jenkins/workspace/github-webhook/cliq-e.py
fi